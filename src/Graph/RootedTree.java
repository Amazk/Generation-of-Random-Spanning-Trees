...
public class RootedTree {
	...
	private class Node {
		...
		private final ArrayList<Node> sons;
		private int height;
		private int size;
		private int depth;
		...
		public void setHeight() {
			int maxHeight = -1;
			for (Node son : this.sons) maxHeight = Math.max(maxHeight, son.height);
		...
		private Node maxSizeSon() {
			Node maxSon = null;
			for (Node son : sons) if (son.size > getSize(maxSon)) maxSon = son;
			return maxSon;
		}
		private Node maxHeightSon() {
			Node maxSon = null;
			for (Node son : sons) if (son.height > getHeight(maxSon)) maxSon = son;
			return maxSon;
		}
		...
		private void print() {
			...
			for (Node son : this.sons) System.out.print(son.vertex + " ");
			System.out.println("(height: " + this.height + ", size: " + this.size
					+ ", 2nd height: " + this.secondMaxHeight() + ", depth: " + this.depth + ")");
		}
	}
	
	// to write recursive algorithms without recursion
	public ArrayList<Node> inverseBfsOrder;
	public ArrayList<Node> bfsOrder;
	public Node nodes[];
	public Node root;
	public int order;
	...
	public int[] getDegreeDistribution(int maxDegree) {
		...
		for (Node n : bfsOrder) {
			...
			if (degree <= maxIndex) degrees[degree]++;
		}
		return degrees;
	}
	...
	private Node getCentroidNode() {
		Node centroid = root;
		while (centroid.maxSizeSon().size * 2 > order) centroid = centroid.maxSizeSon();
		return centroid;
	}
	...
	public double getAverageEccentricity() {
		...
		for (Node n : bfsOrder) sumEccentricity = sumEccentricity + n.depth;
	...
	private void rerootTree() {
		computeAllHeights();
		while (isUnbalanced()) swapRootWith(root.maxHeightSon());
		resetBfsOrdering();
	}
